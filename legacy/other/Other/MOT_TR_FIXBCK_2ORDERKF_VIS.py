...
class MOT():
    def __init__(self,pcap_path,output_file_path,background_update_frame,ending_frame,win_size, eps, min_samples, missing_thred,
                 save_pcd = None, save_Azimuth_Laser_info = False,save_TD_Map = False,save_Labeling_map = False, result_type = 'split', if_vis = False):
        """
    ...
    def mot_tracking(self,A,P_em,H,Q,R):
        ...
        while True: #Iterate Until a frame with one or more targets are detected 
            ...
            if self.save_pcd != 'nosave':
                ...
                self.save_cur_pcd(pcd,Frame_ind)
        ...
        for Frame_ind in pbar:
            ...
            if self.if_vis:
                cur_pcd = self.cur_pcd(Td_map,Labeling_map,self.Tracking_pool)
