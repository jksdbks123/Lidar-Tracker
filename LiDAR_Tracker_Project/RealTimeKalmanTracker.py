...
class MOT():
    def __init__(self,pcap_path,output_file_path,background_update_time,d ,thred_s ,N ,
        ...
        ### Online holder
        self.Td_maps_queue = Manager().Queue(18000) # 200 temp frames
        self.Td_maps_temp = Manager().Queue(18000) 
    ...
    def initialization(self):
        print("Initializing ... ")
        self.traj_path = os.path.join(self.output_path ,'OutputTrajs')
        self.Azimuth_Laser_info_path = os.path.join(self.output_path ,'OutputAzimuthLaserInfo')
        self.Labeling_map_path =  os.path.join(self.output_path,'LabelingMaps')
        self.Td_map_path =  os.path.join(self.output_path ,'TDMaps')

        if 'OutputTrajs' not in os.listdir(self.output_path ):
            os.mkdir(self.traj_path)
        if 'OutputAzimuthLaserInfo' not in os.listdir(self.output_path):
            os.mkdir(self.Azimuth_Laser_info_path)
        if 'LabelingMaps' not in os.listdir(self.output_path):
            os.mkdir(self.Labeling_map_path)
        if 'TDMaps' not in os.listdir(self.output_path ):
            os.mkdir(self.Td_map_path)   
        
        frame_gen = TDmapLoader(self.pcap_path).frame_gen() 
        aggregated_maps = []
        start_time = time.time()
        while True:
            if (time.time() - start_time) > self.background_update_time:
                break
            frame = next(frame_gen)
            
            aggregated_maps.append(frame)
    ...
    def update_bckmap(self,Td_maps_temp):

        start_time = time.time()
        while True:
            if (time.time() - start_time) > self.background_update_time:
                Used_frames = []
                while not self.Td_maps_temp.empty():
                    Used_frames.append(Td_maps_temp.get_nowait())
                aggregated_maps = np.array(Used_frames)
                print("Generating Background Map:", aggregated_maps.shape)
                self.thred_map = gen_bckmap(aggregated_maps , self.d, self.thred_s, self.N , self.delta_thred, self.step)
                start_time = time.time() 

    def reading_frames(self,Td_maps_queue, Td_maps_temp):
        frame_gen = TDmapLoader(self.pcap_path).frame_gen() 
        while True:
            frame = next(frame_gen)
            Td_maps_queue.put(frame)
            Td_maps_temp.put(frame)
    ...
    def run(self):
        self.initialization()
        p_tracking = Process(target = self.mot_tracking, args = [self.Td_maps_queue,])
        p_reading  = Process(target = self.reading_frames, args = [self.Td_maps_queue, self.Td_maps_temp])
        # p_vis = Process(target = self.show_vis, args = [self.pcd_queue])
        p_update = Process(target = self.update_bckmap, args = [self.Td_maps_temp]) 
        p_tracking.start()
        p_reading.start()
        # p_vis.start()
        p_update.start()
