...
db_merge = DBSCAN(eps = 1.5, min_samples = 2)
#xylwh xylwh, xy
...
def get_appearance_features(rows,cols,Td_map): #obtain length height and width
    ...
    #  point_cnt  Dir(x)  dir(y)  Height Len  Width  2Darea dis
...
def get_affinity_IoU(app_cur,app_next,unique_label_next,unique_label_cur,Labeling_map_cur,Labeling_map_next):
    ...
    for i,pair in enumerate(pairs_intersect):
        ...
        if app_cur[cur_ind,4,0] == 0:
            diff_len = 0
        else:
            diff_len = np.abs(app_next[next_ind,4,0] - app_cur[cur_ind,4,0])/app_cur[cur_ind,4,0]
        if diff_len > 0.3:
            continue
        if dis > 2:
            dis = 2 
...
def get_affinity_kalman(failed_tracked_ind,new_detection_ind,state_cur_,mea_next,P_cur_,app_cur,app_next):
    ...
    for i,glb_ind in enumerate(failed_tracked_ind):
        ...
        for j,label_ind in enumerate(new_detection_ind):
            ...
            if app_cur[i,4,0] == 0:
                ...
                diff_len = np.abs(app_cur[i,4,0] - app_next[j,4,0])/app_cur[i,4,0] 
            if diff_len > 0.3:
                continue
