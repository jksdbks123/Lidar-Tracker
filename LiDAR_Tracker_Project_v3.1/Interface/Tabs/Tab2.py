...
from Tracker import *
from ThreadItems import terminate_process
...
def build_tab2(tab, config):
    ...
    output_folder = StringVar(value=config.get_param("tab2")["output_traj_folder"])
    ...
    def listen_to_progress():
        ...
        while True:
            ...
            if progress_update is None:  # Signal to stop listening
                break
            current_progress = progress_var.get()
            progress_var.set(current_progress + progress_update)
    
    Thread(target=listen_to_progress, daemon=True).start()
    ...
    progress_bar = ttk.Progressbar(
        tab, orient="horizontal", mode="determinate", variable=progress_var, maximum=100
    )
    ...
    ttk.Button(
        tab,
        text="Terminate",
        command=lambda: terminate_process(termination_event, progress_var, progress_queue)
    ).grid(column=2, row=6, padx=10, pady=10)
    
    # create a series of entries in the right side of 
    # Buttons
    ttk.Button(
        tab,
        text="Start Batch Processing",
        command=lambda: run_batch_mot_with_ui_threaded(
            batch_folder.get(), 
            output_folder.get(),
