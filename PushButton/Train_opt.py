...
def train_model(device,num_epochs,learning_rate,batch_size,criterion,augmentation_dict,train_folder, val_folder, run_dir):
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(total=len(train_loader), desc="Training") as pbar:
            for i, (frames,optical_flow_frames, labels, location) in enumerate(train_loader):
                frames = frames.to(device)
                optical_flow_frames = optical_flow_frames.to(device)
        ...
        with torch.no_grad():
            with tqdm(total=len(val_loader), desc="Validation") as pbar:
                for i, (frames,optical_flow_frames, labels, location) in enumerate(val_loader):
...
if __name__ == "__main__":
    ...
    batch_size = 2
    run_dir = r"D:\LiDAR_Data\2ndPHB\Video\left_signal_0106"
