...
class MOT():
    ...
    def update_bckmap(self):

        start_time = time.time()
        aggregated_maps = []
        while True: 
            aggregated_maps.append(self.parrnt_conn.recv())
            if (time.time() - start_time) > self.background_update_time:
                aggregated_maps = np.array(aggregated_maps)
                print("Generating Background Map:", aggregated_maps.shape)
                self.thred_map = gen_bckmap(aggregated_maps , self.d, self.thred_s, self.N , self.delta_thred, self.step)
    ...
    def reading_frames(self,Td_maps_queue):
        frame_gen = TDmapLoader(self.pcap_path).frame_gen() 
        while True:
            frame = next(frame_gen)
    ...
    def run(self):
        ...
        p_reading  = Process(target = self.reading_frames, args = [self.Td_maps_queue,])
        # p_vis = Process(target = self.show_vis, args = [self.pcd_queue])
        p_update = Process(target = self.update_bckmap) 
        p_tracking.start()
        p_reading.start()
        # p_vis.start()
        p_update.start()
